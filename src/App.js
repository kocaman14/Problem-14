...
const handleClick = (id)=>{
  let total= carts.filter((cartProduct)=>cartProduct.id !== id)
  setCarts(total)
}

const sum = carts.reduce((total, current) => {
const price = parseFloat(current.price.replace(/[$,]/g, ''))  //burda bu formülü kullandık bu olmasaydı yukarıda sayısal olarak yazıcaktık
return total + price
}, 0)
const shipping = 5.00
      ...
      <div className='mt-4 rounded-lg border border-gray-200 bg-white shadow-sm'>
        ...
        <ul role='list' className='divide-y divide-gray-200'>
          ...
          <div className='flex items-center justify-between'>
            ...
            <dd className='text-sm font-medium text-gray-900'>${sum.toFixed(2)}</dd>
          </div>
          <div className='flex items-center justify-between'>
